...
import { GlobalConfig, OpenRouterModel } from '../ralph/types';

export class OpenRouterPanel {
  private static instance: OpenRouterPanel | undefined;
  ...
  static show(extensionUri: vscode.Uri, config: RalphConfig): void {
    ...
    const panel = vscode.window.createWebviewPanel(
      'ralph.openRouterSettings',
      'Ralph: OpenRouter Settings',
  ...
  private async fetchModels(): Promise<void> {
    ...
    try {
      ...
      const body = await response.json() as { data: Array<{ id: string; name: string; context_length: number; pricing: { prompt: string; completion: string } }> };
  ...
  private toggleModel(modelId: string, enabled: boolean): void {
    const globalConfig = this.config.loadGlobalConfig();
    if (!globalConfig.openrouter) {
      globalConfig.openrouter = {};
    }
  ...
  private setModelSelection(field: 'defaultModel' | 'researchModel', modelId: string): void {
    const globalConfig = this.config.loadGlobalConfig();
    if (!globalConfig.openrouter) {
      globalConfig.openrouter = {};
    }
